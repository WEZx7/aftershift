(() => {
    ...
    function patternTranslate(value) {
        ...
        match = input.match(
            /^(\d+)\s+of\s+8\s+evidence items discovered\.\s*RELAY-09 is\s+(isolated|still active)\.\s*Lena Park identity is\s+(verified|unverified)\.$/i
        );
        if (match) {
            const relay = match[2].toLowerCase() === "isolated" ? "معزول" : "لا يزال نشطًا";
            const lena = match[3].toLowerCase() === "verified" ? "تم التحقق منها" : "لم يتم التحقق منها";
            return `تم اكتشاف ${match[1]} من أصل 8 أدلة. RELAY-09 ${relay}. هوية Lena Park ${lena}.`;
        }

        match = input.match(/^Evidence:\s*(\d+)\s*\/\s*(\d+)$/i);
        if (match) return `الأدلة: ${match[1]} / ${match[2]}`;

        match = input.match(/^Services Restored:\s*(\d+)\s*\/\s*(\d+)$/i);
        if (match) return `الخدمات المستعادة: ${match[1]} / ${match[2]}`;

        match = input.match(/^Critical Services Restored:\s*(\d+)\s*\/\s*(\d+)$/i);
        if (match) return `الخدمات الحرجة المستعادة: ${match[1]} / ${match[2]}`;

        match = input.match(/^ORCH-13:\s*(ISOLATED|ACTIVE)$/i);
        if (match) return `ORCH-13: ${match[1].toUpperCase() === "ISOLATED" ? "معزول" : "نشط"}`;

        match = input.match(/^MANUAL-BUS:\s*(ONLINE|OFFLINE)$/i);
        if (match) return `MANUAL-BUS: ${match[1].toUpperCase() === "ONLINE" ? "متصل" : "غير متصل"}`;

        match = input.match(/^Backup Reserve:\s*(\d+)\s+minutes?$/i);
        if (match) return `احتياطي الطاقة: ${match[1]} دقيقة`;

        match = input.match(/^Attribution:\s*(VALIDATED|UNVALIDATED)$/i);
        if (match) return `الإسناد: ${match[1].toUpperCase() === "VALIDATED" ? "تم التحقق" : "غير متحقق"}`;

        match = input.match(/^NW-ROOT:\s*(ISOLATED|ACTIVE)$/i);
        if (match) return `NW-ROOT: ${match[1].toUpperCase() === "ISOLATED" ? "معزول" : "نشط"}`;

        match = input.match(/^Case Confidence:\s*(\d+)%$/i);
        if (match) return `ثقة القضية: ${match[1]}%`;

        match = input.match(/^Perfect Previous Campaign:\s*(YES|NO)$/i);
        if (match) return `الحملة السابقة المثالية: ${match[1].toUpperCase() === "YES" ? "نعم" : "لا"}`;
